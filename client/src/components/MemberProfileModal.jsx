...
const MemberProfileModal = ({ member, onClose }) => {
  ...
  <div className="flex-1 min-w-0">
    ...
    <div className="flex items-center flex-wrap gap-2 mb-2">
      <h1 className="text-lg sm:text-3xl font-bold text-teal-300 break-words">{member.name}</h1>
    ...
    {/* Contact Info */}
    <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0 text-xs sm:text-sm text-green-400">
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
    ...
    <div className="lg:col-span-2 space-y-4 lg:space-y-6">
      {member.profile?.bio && (
        <div>
          <h2 className="text-lg sm:text-xl font-semibold text-teal-300 mb-2">About</h2>
          <div className="bg-[#111] rounded-lg p-3">
            <p className="text-green-300 text-sm sm:text-base leading-relaxed">{member.profile.bio}</p>
          </div>
        </div>
      )}
      ...
      {member.currentPosition && (
        <div>
          <h2 className="text-lg sm:text-xl font-semibold text-teal-300 mb-2">Current Position</h2>
          <div className="bg-[#111] border border-teal-800 rounded-lg p-3 sm:p-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <h3 className="text-green-400 font-medium text-sm mb-1">Position</h3>
                <p className="text-sm">{member.currentPosition.title}</p>
              </div>
              <div>
                <h3 className="text-green-400 font-medium text-sm mb-1">Department</h3>
                <p className="text-sm capitalize">{member.currentPosition.department}</p>
              </div>
              <div>
                <h3 className="text-green-400 font-medium text-sm mb-1">Start Date</h3>
                <p className="text-sm">{formatDate(member.currentPosition.startDate)}</p>
              </div>
              <div>
                <h3 className="text-green-400 font-medium text-sm mb-1">Academic Period</h3>
                <p className="text-sm">{member.currentPosition.period}</p>
              </div>
            </div>
            {member.currentPosition.responsibilities?.length > 0 && (
              <div className="mt-3">
                <h3 className="text-green-400 font-medium text-sm mb-2">Responsibilities</h3>
      ...
      {member.positionHistory?.length > 0 && (
        <div>
          <h2 className="text-lg sm:text-xl font-semibold text-teal-300 mb-2">Position History</h2>
              ...
              <div key={i} className="bg-[#111] border border-teal-800 rounded-lg p-3 sm:p-4">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1">
                    <h3 className="text-green-200 font-medium text-sm">{pos.title}</h3>
                    <p className="text-green-400 text-xs capitalize">{pos.department}</p>
                  </div>
                  <span className={`ml-2 px-2 py-1 text-xs rounded-full font-medium ${getRoleColor(pos.role)}`}>
                    {pos.role?.replace('-', ' ')}
                  </span>
                </div>
                <div className="flex flex-wrap text-xs text-green-500 mb-2 gap-2">
                ...
                {pos.responsibilities?.length > 0 && (
                  <div>
                    <p className="text-green-300 text-xs font-medium mb-1">Key Responsibilities:</p>
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="bg-[#111] border border-teal-800 rounded-lg p-3 sm:p-4">
        <h3 className="text-teal-300 font-semibold text-base sm:text-lg mb-2">Member Stats</h3>
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-green-400">Total Positions</span>
      ...
      {member.profile?.skills?.length > 0 && (
        <div className="bg-[#111] border border-teal-800 rounded-lg p-3 sm:p-4">
          <h3 className="text-teal-300 font-semibold text-base sm:text-lg mb-2">Skills</h3>
          <div className="flex flex-wrap gap-2">
            {member.profile.skills.map((skill, i) => (
              <span key={i} className="bg-green-800 text-green-200 px-2 py-1 text-xs rounded-full">{skill}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {member.profile?.experience?.level && (
        <div className="bg-[#111] border border-teal-800 rounded-lg p-3 sm:p-4">
          <h3 className="text-teal-300 font-semibold text-base sm:text-lg mb-2">Experience</h3>
          <div className="text-sm space-y-2">
            ...
            {member.profile.experience.yearsInAnimation && (
              <div className="flex justify-between">
                <span className="text-green-400">Years in Animation</span>
      ...
      {member.visibility?.contactAllowed && (
        <div className="bg-[#111] border border-teal-800 rounded-lg p-3 sm:p-4">
          <h3 className="text-teal-300 font-semibold text-base sm:text-lg mb-2">Contact</h3>
          <div className="text-sm space-y-2">
