...
const Navbar = () => {
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-black/60 backdrop-blur-md border-b border-b-[0.5px] border-emerald-500 shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center  h-16">
          ...
          {/* Logo */}
          <Link to="/" className="flex left-align space-x-3">
            {settings.logo ? (
              <img 
                src={settings.logo} 
            ...
            <span className="text-emerald-400 font-extrabold text-xl glow-neon tracking-wide">
  ...
  <UserNav />
      ...
      {/* Mobile Nav */}
      {isMenuOpen && (
        <div className="lg:hidden bg-[#0b0f11] border-t border-emerald-600 px-4 pb-4 pt-2 space-y-2">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              onClick={() => setIsMenuOpen(false)}
              className={`block text-base font-medium rounded-md px-3 py-2 transition ${
                isActive(link.path)
